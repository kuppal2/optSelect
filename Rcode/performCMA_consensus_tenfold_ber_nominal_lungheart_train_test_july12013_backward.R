...
performCMA<-function(ncid, testm, outloc, maxnum, minnum, stepitr, gsmethods, percentTest,featweight, accuracyweight, kname, maxitrs, minpresent, norm_method, tolerance=1, classindex, upclasssep, dclasssep, removeindex, numfacts,numfolds=10,evalmethod="CV",CVfoldthresh=0.3,backward.sel=FALSE)
{
...
train<-GenerateLearningsets(y=nci_y, method=evalmethod, fold=numfolds, strat=TRUE)
...
#if(FALSE)
{
varsel <- GeneSelection(nci_xm,nci_y, learningsets = train, method=c("t.test"),scheme="pairwise")
	genelist={}
	for(i in 1:numfolds)
	{
...
for(m in 1:length(gsmethods))
{
	...
	#varsel <- GeneSelection(nci_xm,nci_y, learningsets = train, method=gsmethods[m],scheme=scheme_val[2])
	genelist={}
	for(i in 1:numfolds)
	{
	...
	genelist_seltable<-genelist_seltable[which(genelist_seltable>=(CVfoldthresh*numfolds))]
