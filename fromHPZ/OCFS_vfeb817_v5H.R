...
accuracyweightB=1,accuracyweightC=1,accuracyweightD=0,max_num_feats=10,num_itrs=3)
{
	...
	for(i in 1:10){
	    set.seed(321*i)
            ...
            #	folderror<-mean(folderror)-(1.96*(sd(folderror)/sqrt(3)))
            folderror_test2<-mean(svm_acc[i]) #reverse
        }
...
for(m in 1:length(gsmethods))
{
       ...
       #         model<-svm_cv(v=numfolds,x=trainset,y=nci_y,kname=kname,errortype="BER")
		...
		tenfoldacc<-{}
                for(r1 in 1:10)
                {
                        seednum=seed_vec[1]*r1
			model<-svm_cv(v=numfolds,x=trainset,y=nci_y,kname=kname,errortype="BER")
	                tenfoldacc<-c(tenfoldacc,model$confint[1])
                        ...
                        #folderror_cur<-model$confint[1] #mean_acc
                        #folderror<-c(folderror,folderror_cur)
                        permtenfoldacc<-model$confint[2] #model$mean_acc #tot.accuracy
                        perm_acc<-c(perm_acc,permtenfoldacc)
                }
		

                perm_acc<-mean(perm_acc,na.rm=TRUE)+2*sd(perm_acc,na.rm=TRUE)
                foldacc<-mean(tenfoldacc,na.rm=TRUE)-2*sd(perm_acc,na.rm=TRUE)
                #fitfunc<-accuracyweight*(foldacc) #+featweight*(1-num_g/dim(nci_xm)[2])

                bestmod<-0.7*(foldacc-perm_acc)+0.3*(foldacc) #-(100*featweight*(1-num_g/dim(nci_xm)[2]))

		print(paste(gsmethods[m],":",length(bestgenelist), " : ", bestmod," : ",foldacc," : ",perm_acc,sep=""))
	...
	acc_vec<-c(acc_vec,bestmod)
	scoringmatrix[bestgenelist,m]=1
	}
}

min_acc_val<-min(acc_vec,na.rm=TRUE)

acc_mat<-cbind(gsmethods,acc_vec)
acc_mat<-as.data.frame(acc_mat)
print(acc_mat)
if(length(gsmethods)>1){
#median_acc_vec<-median(acc_vec,na.rm=TRUE)

firstquartile_val<-quantile(acc_vec,0.25)

bad_methods<-which(acc_vec<firstquartile_val) #median_acc_vec)
...
print(ki_res)
		...
		fitness_res<- eval_fit_test_diff(particle=x,numfolds=numfolds,trainm=trainm,trainclass=trainclass,
                                                ...
                                                accuracyweightB=weightB,accuracyweightC=weightC,accuracyweightD=weightD,featweight=featweightcur,max_num_feats=maxnum,num_itrs=100)
                print(fitness_res)
