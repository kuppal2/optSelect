 ...
 #function to evaluate k-fold CV
	...
	accuracyweightB=1,max_num_feats=10)
	{
		...
		if(num_feat>1)
		{
			...
			#print(model)
			#print(folderror_cur)	
			folderror_cur_reg<-c(folderror_cur_reg,folderror_cur) #model$tot.accuracy)
			set.seed(seed_vec[f])
			rand_ind<-sample(x=seq(1,length(trainset$trainclass)),size=length(trainset$trainclass))
                        model<-svm_cv(v=numfolds,x=trainset[,-1],y=trainset$trainclass[rand_ind],kname=kname,errortype=errortype,setseed=setseed)
...
percentTrain=(1-percentTest)
#set.seed(321)

train<-GenerateLearningsets(y=trainclass, method=evalmethod, fold=numfolds, strat=FALSE, niter=iter_learn,ntrain=(percentTrain*nrow(trainm)))
...
for(m in 1:length(gsmethods))
{
	...
	print(head(genelist_seltable))
	#select genes that were selected in at least CVfoldthresh iterations

	check_minf<-which(genelist_seltable>=(CVfoldthresh*numfolds*iter_learn))
	if(length(check_minf)>0){	
	genelist_seltable<-genelist_seltable[which(genelist_seltable>=(CVfoldthresh*numfolds*iter_learn))]
	}else{

		check_minf<-which(genelist_seltable>(0))
			if(length(check_minf)>0){
		 genelist_seltable<-genelist_seltable[which(genelist_seltable>(0))]
		}else{
			next
		}
	}
