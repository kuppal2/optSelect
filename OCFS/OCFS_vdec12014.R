		...
		if(bootstrap_val==TRUE)
			...
			else
			{
				...
				if(no_change>maxitrreset)
				{
					...
					len_feat_ind<-apply(x_lbest,2,mean)	

					if(global_no_change>global_max_itr & length(which(x_lbest>0))>minnum & (length(which(len_feat_ind>=min.select.pct))>=length(which(x_gbest==1)-1))){
