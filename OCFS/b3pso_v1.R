	...
	featweightcur<-featweight.min+(((featweight.max-featweight.min)/k))				
		...
		if(evalMode=="bootstrap")
			...
			print(boot_fitness)
				...
				res1<-clusterApply(cl,part_list,evalFunc,trainm=trainm,trainclass=trainclass,numfolds=numfolds,errortype=errortype,accuracyweightA=accuracyweightA,accuracyweightB=accuracyweightB,featweight=featweightcur,maxnum=maxnum,kname=kname)
