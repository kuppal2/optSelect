...
#testm<-testm[,-c(1,3,30,31)]
#ncid<-ncid[,-c(1,3,30,31)]

ncid<-read.csv("Output/PSO_reg_results/itr13_IMT/modified_train.csv")
testm<-read.csv("Output/PSO_reg_results/itr13_IMT/modified_test.csv")
#filestr3<-paste(outloc, "original_testdata.csv", sep="")
#write.table(testm, file=filestr3, sep=",", row.names=FALSE)
...
meantrainimt=mean(ncid$Class)
write.csv(meantrainimt, paste(outloc,"mean_imttrain.csv",sep=""), row.names=FALSE)

upsep<-meantrainimt+0.05
dsep<-meantrainimt-0.05

#meantrainimt=0.68

#ncid$Class[which(ncid$Class>meantrainimt)]="A"
#ncid$Class[which(ncid$Class<=meantrainimt)]="B"

#testm$Class[which(testm$Class>meantrainimt)]="A"
#testm$Class[which(testm$Class<=meantrainimt)]="B"

ncid$Class[which(ncid$Class>=upsep)]="A"


#ncid<-ncid[which(ncid$Class=="A" || ncid$Class=="B"),]

ncid$Class[which(ncid$Class<=dsep)]="B"
ncidA<-ncid[which(ncid$Class=="A"),]
ncidB<-ncid[which(ncid$Class=="B"),]
ncid<-rbind(ncidA,ncidB)
print(levels(ncid))
testm$Class[which(testm$Class>=upsep)]="A"
testm$Class[which(testm$Class<=dsep)]="B"
testmA<-testm[which(testm$Class=="A"),]
testmB<-testm[which(testm$Class=="B"),]
...
train.table<-table(pred10fold,nci_y)
