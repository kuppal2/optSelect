...
svm_acc[i]<-(beracc*100)
	...
	accuracyweightB=1,max_num_feats=10)
	{
		...
		if(num_feat>2)
		{
			...
			setseed=NA #seed_vec[3]
                        model<-svm_cv(v=numfolds,x=trainset[,-1],y=trainset$trainclass,kname=kname,errortype=errortype)
                        foldacc<-model$confint[1]
		

                perm_acc<-{}
               seed_vec<-c(129532,839147,407700,99999,351)
		...
		perm_acc<-lapply(1:num_itrs,function(r1)
			...
			set.seed(seed_vec[r1])
	                ...
	                permtenfoldacc<-model$confint[2] #tot.accuracy
                        #perm_acc<-c(perm_acc,permtenfoldacc)
             		return(permtenfoldacc)
		   })
		perm_acc<-unlist(perm_acc)
		...
		folderror<-rep(foldacc,num_itrs)
...
svm_acc[i]<-(beracc*100)
		...
		t1<-t.test(x=folderror,y=folderror_perm,paired=T,alternative="greater")
                                ...
                                folderror<-mean(folderror)#+(runif(1,(-5),(-1)))
                                if(num_feat>max_num_feats){
                                fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror)-(featweight*100*(num_feat/length(particle))) #(featweight*(num_feat)) 
                                }else{
                                        fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror)
...
return(list(modgenelist=bestgenelist, modtraindata=modtrain, modtestdata=modtest, blindtest=testacc, modtrainclass=nci_y, modtestclass=test_y, numfeat=dim(modtrain)[2], testacc=testacc, tenfoldacc=model_train$tot.accuracy,learningsets=train@learnmatrix,boostweight=boostweight))
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,evalFunc,boostweight=NA,...)
{
...
print(ki_res)
		...
		bestgenelist_median=which(summat>=num_itr_thresh) 	
		
		print(paste("Number of features selected in ",num_itr_thresh," iterations based on median:", sep=""))
                print(length(bestgenelist_median))

		num_itr_thresh<-ceiling(minselect.pct*globalpso_maxitr)
		bestgenelist_minsel=which(summat>=num_itr_thresh)  #ceiling(minselect.pct*globalpso_maxitr))

		print(paste("Number of features selected in ",num_itr_thresh," iterations based on min sel:", sep=""))
                print(length(bestgenelist_minsel))
		...
		print(paste("Number of features selected based on validation:", sep=""))
                print(length(bestgenelist))

		feat_ind<-bestgenelist#_median
...
return(list(scoringmatrix=scoringmatrix,bestfeatlist=bestgenelist,bestfeatnames=feat_names,model=mod,trainm=trainm,testm=testm))
...
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,seednum=NA,varnames=NA, itr.terminate=TRUE,evalFunc,trainm=NA,trainclass=NA,boostweight=NA,...)
{
...
print("here")
  scoringmatrix=matrix(0,dimsize,globalpso_maxitr)
	...
	fit_res<-eval_fit_test_diff(particle=rep(1,dim(trainm)[2]),numfolds=numfolds,trainm=trainm,trainclass=trainclass,
                                                testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,accuracyweightA=accuracyweightA,
                                                accuracyweightB=accuracyweightB,featweight=0.01,max_num_feats=maxnum)

	fit1<-(-1)*fit_res$fitfunc
	fitness_x<-array(fit1, dim=c(num_part, 1)) 
	 fitness_gbest<-fit1 #100000000000000 
	cverror_gbest<-(fit_res$cverror)
	cvpermerror_gbest<-(fit_res$cvpermerror)
	prev_gbest<-fitness_gbest+1 
	 fitness_lbest<-array(fit1, dim=c(num_part, 1))
