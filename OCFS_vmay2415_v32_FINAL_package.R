...
performCMA<-function(trainm, trainclass, testm, testclass, outloc, maxnum, minnum, stepitr, gsmethods, pct_train=1,
                     featweight, accuracyweight, kname, maxitrs, minpresent, norm_method, tolerance=1, classindex, 
                     numfacts,numfolds=10,evalmethod="CV",CVfoldthresh=0.3,varselmethod="backward", 
                     scheme_val="one-vs-all",iter_learn=5,boostweight=NA,RankAggreg.run=FALSE)
{
...
data_dim<-dim(trainm)
write.csv(testclass, paste(outloc,"testclass.csv",sep=""), row.names=FALSE)
write.csv(trainclass, paste(outloc,"trainclass.csv",sep=""), row.names=FALSE)
...
print(paste("10 fold train accuracy using RankAggreg-CE", model_trainRA$tot.accuracy, sep=""))
print(paste("Test AUC using RankAggreg-CE:", auc_acc, sep=""))
...
bestgenelistGA<-gsub(x=r1$top.list,pattern="var",replacement="")
bestgenelistGA<-as.numeric(as.character(bestgenelistGA))
modtrainRA<-as.matrix(nci_xm[,c(bestgenelistGA)])
modtestRA<-as.matrix(test_xm[,c(bestgenelistGA)])
model_train_valid<-svm(modtrainRA,  nci_y,   kernel=kname, type="C")
pred_test<-predict(model_train_valid, modtestRA)
test.table<-table(pred_test, test_y)
testacc<-sum(diag(test.table))/(dim(modtestRA)[1])
...
#print("Num itr RA GA")
#print(r1$num.iter)
print(paste("10 fold train accuracy using RankAggreg-GA:", model_trainRA$tot.accuracy, sep=""))
print(paste("Test AUC using RankAggreg-GA:", auc_acc, sep=""))
...
return(list(modgenelist=bestgenelist, modtraindata=modtrain, modtestdata=modtest, 
            blindtest=testacc, modtrainclass=nci_y, modtestclass=test_y, numfeat=dim(modtrain)[2], 
            testacc=testacc, tenfoldacc=model_train$tot.accuracy,learningsets=train@learnmatrix,boostweight=boostweight,
            scoringmatrix=scoringmatrix,bestgenelistRA=bestgenelistRA,bestgenelistGA=bestgenelistGA))
...
run_pso<-function(trainm,trainclass,testm,testclass,outloc,maxnum=5,
...
return(list(scoringmatrix=scoringmatrix,bestfeatlist=bestgenelist,bestfeatnames=feat_names,
            train.svm.model=mod,trainm.new=trainm,testm.new=testm,trainacc=trainacc,testacc=testacc,testauc=auc_acc,
