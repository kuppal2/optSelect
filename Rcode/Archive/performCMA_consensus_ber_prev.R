...
performCMA<-function(datafile, outloc, maxnum, minnum, step, methods, percentTest,featweight, accuracyweight, kname, maxitrs, minpresent)
{
...
scoringmatrix=matrix(0,data_dim[2]-1,length(methods))
...
for(m in 1:length(methods))
{
	...
	while(num_g<maxnum)
	{
		...
		svm.table<-table(model$fitted, nci_y)

                ind_class_A<-which(nci_y==classA)
...
ind_class_B<-which(nci_y==classB)
...
foldacc<-1-ber
		...
		prevacc=fitfunc
		#prevacc=foldacc
		print(paste(methods[m],":",num_g, ":", fitfunc,sep=""))
...
bestgenelist=which(summat==minpresent[length(minpresent)])


if(length(bestgenelist)<1)
{
	#minpresent=minpresent-1
	bestgenelist=which(summat>=minpresent[length(minpresent)-1])
	{
		if(length(bestgenelist)<1)
		{
			#minpresent=minpresent-1
			bestgenelist=which(summat==minpresent[length(minpresent)-2])
		}
		else
		{
