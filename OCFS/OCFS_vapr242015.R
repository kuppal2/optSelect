 ...
 #function to evaluate k-fold CV
	...
	accuracyweightB=1,max_num_feats=10)
	{
		...
		if(num_feat>1)
		{
			...
			folderror_reg<-{}
			#if(FALSE)
			seed_vec<-c(129532,839147,407700,100,321)
			{
			 for(f in 1:5)
                        {
...
svm_acc[i]<-(beracc*100)
		...
		folderror<-svm_acc[i]
			...
			#se<-(1.96*(sd(folderror_perm,na.rm=TRUE)/sqrt(2)))
			me_perm <- qt(.975,4)*sd(folderror_perm,na.rm=TRUE)/sqrt(5)

                                folderror_perm<-mean(folderror_perm,na.rm=TRUE)+me_perm

			me_reg <- qt(.975,4)*sd(folderror_reg,na.rm=TRUE)/sqrt(5)       
				folderror_reg<-mean(folderror_reg,na.rm=TRUE)-me_reg  #(1.96*(sd(folderror_perm,na.rm=TRUE)/sqrt(2))) 
	                        ...
	                        if(num_feat>max_num_feats){
                                fitfunc<-(accuracyweightA*(folderror_reg-folderror_perm))+accuracyweightB*(folderror)-(featweight*100*(num_feat/length(particle))) #(featweight*(num_feat))
                                        ...
                                        fitfunc<-(accuracyweightA*(folderror_reg-folderror_perm))+accuracyweightB*(folderror)
                                }
...
error <- qt(.975,(length(svm_acc)-1))*(sd_acc/sqrt(length(svm_acc)))
...
alltrainm<-trainm
alltrainclass<-as.factor(trainclass)
...
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,seednum=NA,varnames=NA, itr.terminate=TRUE,evalFunc,trainm=NA,percentTrain=0.7,trainclass=NA,...)
{
...
numtrain<-(percentTrain*nrow(trainm))
evalmethod='MCCV'
set.seed(321)
print("s")
print(numtrain)
...
print("here")
  scoringmatrix=matrix(0,dimsize,globalpso_maxitr)
	...
	if(dim(alltrainm)[1]>100){

		bootsize=dim(alltrainm)[1]
	}else{
	...
	for (row in 1:num_part)
	 {
		...
		if(agent_behavior[row]==3){
			...
			x[row,]<-rep(1,length(x_lbest[row,]))
                                x_lbest[row,]<-rep(1,length(x_lbest[row,]))
			num_feat<-length(x_lbest[row,])
		}else{
		...
		if(evalMode=="bootstrap")
			{
				...
				for(boot_itr in 1:bootstrap.itr)
				{
                		...
                		subtrain_ind<-sample(x=seq(1,dim(trainm)[1]),size=bootsize,replace=TRUE)
                               subtrain<-trainm[subtrain_ind,]
                               subtrainclass<-trainclass[subtrain_ind]
                        print(head(trainm))
                        print(head(trainclass))
				...
				boot_fitness<-apply(boot_fitness,1,function(x){mean(x,na.rm=TRUE)+(qt(0.975,bootstrap.itr-1)*(sd(x,na.rm=TRUE)/sqrt(bootstrap.itr)))}) #mean(boot_fitness,na.rm=TRUE)
