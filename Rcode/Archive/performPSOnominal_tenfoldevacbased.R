...
{	
	...
	nn_search<-function(particle, part_group, group_fitness, numparticles, neighbdist)
	{
        partm<-as.matrix(particle)
	...
	fitness_order<-group_fitness[nn_order[1:neighbdist]]

	best_neighbfitness<-min(fitness_order)
	
	best_neighbindex<-which(group_fitness[nn_order[1:neighbdist]]==best_fitness)

        return(best_neighbindex)
	}
