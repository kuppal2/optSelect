...
nn_search<-function(particle_ind, part_group)
{
 ...
 #function to evaluate k-fold CV
	...
	#function to evaluate k-fold CV
	eval_fit_test_diff<-function(particle, numfolds,trainm,trainclass,testm,testclass,errortype="AUC",kname="radial",featweight=0.05,accuracyweight=1)
	{
		...
		if(bootstrap_val==TRUE)
			{
				...
				res1<-clusterApply(cl,part_list,eval_fit_test_diff,numfolds=numfolds,trainm=subtrain,trainclass=subtrainclass,
						testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,accuracyweight=accuracyweightB,featweight=featweightcur)
			...
			else
			{
				...
				if(no_change>maxitrreset)
				{
					...
					d1<-dist(as.matrix(rbind(x_lbest_vec,overall_x_gbest)))^2
						...
						if(d1pct<=10 | global_no_change>(global_max_itr*2))
						{	
					#	print(dim(x_lbest))	
