...
accuracyweightB=1,accuracyweightC=1,accuracyweightD=0,max_num_feats=10)
{
    ...
    if(num_feat>2)
    {
        ...
        #if(num_feat>max_num_feats)
        {
            ...
            fitfunc<-(accuracyweightA*(trainfolderror-trainfolderror_perm)^2)+(accuracyweightB*(testacc))+(accuracyweightC*(folderror_test2))-(accuracyweightD*(trainfolderror_perm)^2) #-(featweight*100*(num_feat/length(particle)))
...
xtemp<-rep(1,dim(trainm)[2])
                fitness_res<- eval_fit_test_diff(particle=xtemp,numfolds=numfolds,trainm=trainm,trainclass=trainclass,
...
alltrainclass<-trainclass
  ...
  scoringmatrix=matrix(0,dimsize,globalpso_maxitr)
	...
	fitness_res<-fitness_res$fitfunc
	fitness_x<-array(fitness_res,dim=c(num_part, 1)) #array(100000000000000000, dim=c(num_part, 1)) 
	 fitness_gbest<-fitness_res #100000000000000 
	cverror_gbest<-(-100000)
	cvpermerror_gbest<-(-100000)
	prev_gbest<-fitness_gbest+1 
	 fitness_lbest<-array(fitness_res,dim=c(num_part, 1)) #array(100000000000000000, dim=c(num_part, 1))
	 ...
	 feat_names<-varnames #colnames(trainm)
		...
		min_fit_x<-fitness_res #1000000000000000
