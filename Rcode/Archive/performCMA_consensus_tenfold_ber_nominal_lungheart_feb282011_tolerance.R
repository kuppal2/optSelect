...
if(FALSE)
{
print(ncid[1:5,c(1,classindex)])
meantrainimt=mean(ncid[,classindex])
print("mean is ")
print(meantrainimt)

print("train class is")
write.csv(meantrainimt, paste(outloc,"mean_classtrain.csv",sep=""), row.names=FALSE)

upsep<-0.68 #meantrainimt+upclasssep
dsep<-0.679 #meantrainimt-dclasssep
...
for(m in 1:length(gsmethods))
{
	...
	while(num_g<maxnum)
	{
		...
		foldacc<-tenfoldacc	
		#foldacc<-0.5*(model$tot.accuracy+beracc)
