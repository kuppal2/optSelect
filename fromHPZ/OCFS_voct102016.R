...
fitness_vec=rep(0,length(gsmethods))
	...
	while(num_g<=maxnum)
		...
		maxnum=min(length(genelist),maxnum)
			...
			bestgenelist=genelist[1:maxnum]
			 trainset<-nci_xm[,genelist[1:maxnum]]			
             #model<-svm(trainset, nci_y, type="C", kernel=kname, cross=numfolds)
            #tenfoldacc<-model$tot.accuracy
			...
			print(paste(gsmethods[m],":",length(bestgenelist), ":", fitfunc,sep=""))
            
            fitness_vec[m]<-fitfunc
...
fitness_vec<-round(fitness_vec)

fitness_vec_summary<-summary(fitness_vec)

bad_methods<-which(fitness_vec<fitness_vec_summary[3])
...
if(length(gsmethods)>1){
    scoringmatrix[,c(bad_methods)]=0
}
summat=apply(scoringmatrix,1,sum)
print("dim of scoring matrix is ")
print(dim(scoringmatrix))
...
if(length(gsmethods)>1){
    ...
    if(length(bad_methods)>0){
        ...
        scoringmatrix<-scoringmatrix[,-c(bad_methods)]
    }

m1<-scoringmatrix

for(i in 1:dim(scoringmatrix)[2]){if(length(which(m1[,i]==1))>0){m2[[i]]<-paste("var",which(m1[,i]==1),sep="")}}
print(m2)
m3<-as.data.frame(m2)
m4<-t(m3)
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,evalFunc,boostweight=NA,train.pct=0.8,min.iter.select=2,...)
{
...
boostweight=boostweight,train.pct=train.pct,min.iter.select=min.iter.select,...)
...
print(paste("Using all features:", sep=""))
x<-rep(1,length(summat))
...
accuracyweightB=weightB,accuracyweightC=weightC,accuracyweightD=weightD,featweight=featweight.min,max_num_feats=3)
...
print(fitness_res)
	...
	print(length(summary))
		...
		median_results<-apply(scoringmatrix,1,median)
		bestgenelist<-which(median_results==1)
        
        
       
        
        
		print(paste("Number of features selected based on median criteria:", sep=""))
                print(length(bestgenelist))
...
#                fitness_res<- eval_fit_test_diff(particle=x,numfolds=numfolds,trainm=trainm,trainclass=trainclass,
 #                                               testm=trainm,testclass=trainclass,errortype=errortype,kname=kname,accuracyweightA=weightA,
  ...
  fitness_res<- eval_fit_test_diff(particle=x,numfolds=numfolds,trainm=trainm,trainclass=trainclass,
                ...
                print(fitness_res)
                        if(fitness_res<=(min_fitness_res)){
...
if(is.na(min.iter.select)==FALSE){

				num_itr_thresh=min.iter.select
...
accuracyweightB=weightB,accuracyweightC=weightC,accuracyweightD=weightD,featweight=featweight.min,max_num_feats=3)
		  ...
		  best_thresh<-num_itr_thresh
				...
				stop(paste("No features selected at ",num_itr_thresh," iterations threshold", sep="")) 
			}
...
print(testacc)
		...
		#global.summary.method="max.iterations"

		#if(global.summary.method=="max.iterations"){
			
		#		best_results<-best_resultsA
		#		best_thresh<-best_threshA
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,seednum=NA,varnames=NA, itr.terminate=TRUE,evalFunc,trainm=NA,trainclass=NA,boostweight=NA,train.pct=0.8,min.iter.select=2,...)
{
