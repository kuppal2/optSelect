...
num_perclass<-table(nci_y)

min_numperclass<-min(num_perclass)

#if(min_numperclass<30){
    
    #   evalmethod="bootstrap"
    #iter_learn=10
    #}
...
set.seed(321)
train<-GenerateLearningsets(y=nci_y, method=evalmethod, fold=numfolds, strat=TRUE, niter=iter_learn,ntrain=(pct_train*nrow(nci_xm)))

}else{
    if(evalmethod=="bootstrap"){
...
for(m in 1:length(gsmethods))
{
    ...
    # load("cma.Rda")
    print(paste("Method name: ",gsmethods[m],sep=""))
    print(dim(nci_xm))
    print(table(nci_y))
    print(train)
    print(scheme_val)
	...
	for(i in 1:(max_i))
	{
		...
		temp_genelist<-c(temp_genelist,t1[[c]]$index)
		}else{
			temp_genelist<-c(temp_genelist,t1$index)
		}
        
		}
        temp_genelist<-unique(temp_genelist)
        
        genelist<-c(genelist,temp_genelist)
	...
	#maxitrs=100
	num_g=length(genelist) #min(length(genelist),maxnum)
	while(num_g>minnum)
	{
        ...
        if(FALSE)
        {
        nested_acc<-lapply(1:dim(trainlearningsets)[1],function(lsetnum){
       
        subtrain<-nci_xm[trainlearningsets[lsetnum,],]
        subtrainclass<-nci_y[trainlearningsets[lsetnum,]]
        subtest<-nci_xm[-c(trainlearningsets[lsetnum,]),]
        subtestclass<-nci_y[-c(trainlearningsets[lsetnum,])]
        
        cparticle=rep(0,dim(subtrain)[2])
        cparticle[genelist[1:num_g]]<-1
        
         nested_acc<-eval_fit_test_diff(particle=cparticle,numfolds=numfolds,trainm=subtrain,trainclass=subtrainclass,
        testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,accuracyweightA=0.7,
        accuracyweightB=0.2,accuracyweightC=0.05,accuracyweightD=0.05,featweight=0.01,max_num_feats=maxnum)
        
        return(abs(nested_acc$fitfunc))
        })
        
        fitfunc<-mean(unlist(nested_acc))
        }
		...
		fitfunc<-accuracyweight*(foldacc-perm_acc)+1*(foldacc)-(100*featweight*(1-num_g/dim(nci_xm)[2]))
        } ###uses svm_cv function
        
        
        if(fitfunc>(bestmod+tolerance))
        {
            bestmod<-fitfunc
	...
	print(paste(gsmethods[m],":",length(bestgenelist), ":", bestmod," : ",num_g,sep=""))	
		...
		if(varselmethod=="forward"){
            ...
            #min(length(genelist),maxnum)
            tempmaxnum=max(length(genelist),maxnum)
            
        while(num_g<=tempmaxnum)
        {
                trainset<-nci_xm[,genelist[1:num_g]]
                # model<-svm(trainset, nci_y, type="C", kernel=kname, cross=numfolds)
                #tenfoldacc<-model$tot.accuracy
                ...
                nested_acc<-lapply(1:dim(trainlearningsets)[1],function(lsetnum){
                    ...
                    print(dim(subtest))
                    print(dim(subtrain))
                    print(length(subtrainclass))
                    print(length(subtestclass))
        ...
        print(paste(gsmethods[m],":",length(bestgenelist), ":", bestmod,sep=""))
            ...
            tempmaxnum=length(genelist) #min(length(genelist),maxnum)
			if(varselmethod=="none"){
			bestgenelist=genelist[1:tempmaxnum]
			}else{
                
                tempmaxnum=min(length(genelist),maxnum)
                if(varselmethod=="maxnum"){
                    bestgenelist=genelist[1:tempmaxnum]
                }else{
    ...
    print(paste("Method: ",gsmethods[m],sep=""))
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,evalFunc,boostweight=NA,train.pct=0.8,...)
{
...
print(ki_res)
		...
		bestgenelist=which(summat>=best_thresh)  #ceiling(minselect.pct*globalpso_maxitr))
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,seednum=NA,varnames=NA, itr.terminate=TRUE,evalFunc,trainm=NA,trainclass=NA,boostweight=NA,train.pct=0.8,...)
{
