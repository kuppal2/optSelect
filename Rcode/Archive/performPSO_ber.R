...
performPSO<-function(trainm, testm, trainclass, testclass, outloc, numitrs, reinititrs, numpart, c1, c2, kname, percentTest, featweight, accuracyweight, cmaacc)
{
	...
	print("Test class")
	print(testclass[1:5])
	all_names<-names(trainm)
	feat_names<-names(trainm)
	data_dim<-dim(trainm)
	...
	eval_fit_kfold<-function(particle, k)
	{
		...
		if(num_feat>=1)
		{
			...
			fitval<-fitted(model)
                	...
                	tab1=table(fitval, trainclass)
                	foldacc=(tab1[1,1]/sum(tab1[,1]))+(tab1[2,2]/sum(tab1[,2]))
                	folderror=0.5*foldacc
	...
	prev_gbest<-fitness_gbest+1
	 fitness_lbest<-array(fitness_gbest, dim=c(num_part, 1))
