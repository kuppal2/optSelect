...
outloc<-"/home/stu/kuppal3/Research/Feature_selection/Datasets/Khan_SRBCT/itr1pso1000f0.6c0.1behav10itrbackwardtrue_cvfoldthres0.7w10725/"
...
maxitrreset<-0.3*itr
num_neighbors<-5
max_num_feats<-0.1*dim(traind[,-c(1)])[1]
##################
...
nn_search<-function(particle_ind, part_group)
{
	...
	eval_fit_kfold_diff<-function(particle, numfolds,trainm,trainclass,testm,testclass,errortype="AUC")
	{
	...
	for (k in 1:itr)
		{
			...
			for (i in 1:num_part)
			{
				...
				res1<-clusterApply(cl,part_list,eval_fit_kfold_diff,numfolds=numfolds,trainm=subtrain,trainclass=subtrainclass,
							testm=subtest,testclass=subtestclass,errortype="CV")
				fitness_x<-sapply(res1,head)
				boot_fitness<-c(boot_fitness,fitness_x)
				}
				
				#boot_fitness<-mean(boot_fitness,na.rm=TRUE)
