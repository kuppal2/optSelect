...
outloc<-"/home/stu/kuppal3/Research/Feature_selection/Datasets/Khan_SRBCT/itr1pso1000f0.2c0.2_behav10itrbackwardtrue_cvfoldthres0.7_wlinctest/"
...
find_similar_samples<-function(part_group, particle_ind=NA){
	...
	dist.matrix<-ann(ref=as.matrix(bcdata), target=as.matrix(targetdata), tree.type="brute", k=3, split.rule="midpt", shrink.rule="centroid", verbose=TRUE)
	...
	for (k in 1:itr)
		{
			...
			else
			{
			 ...
			 ###update the particle velocity and position
				...
				nn_search_res<-find_similar_samples(x,NA)
			...
			for (i in 1:num_part)
			{
                ...
                {                               
                        ...
                        if (ran[col]<0)         
                        {
							...
							#print(x_curbest_ind)
				                	if(num_neighbors>1){
							x_curbest<-apply(x[x_curbest_ind,],2,mean)
							}else{
