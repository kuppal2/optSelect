 ...
 #function to evaluate k-fold CV
		...
		folderror<-{}
			...
			#	folderror<-mean(folderror,na.rm=TRUE)  #-(1.96*(sd(folderror,na.rm=TRUE)/sqrt(4)))
				...
				fitfunc<-(accuracyweight*(0.005*folderror)*(folderror-folderror_perm))-(featweight*100*(num_feat/length(particle)))
				}else{

					#fitfunc<-(accuracyweight*(folderror)*(folderror-folderror_perm))
					fitfunc<-(accuracyweight*(0.005*folderror)*(folderror-folderror_perm))
