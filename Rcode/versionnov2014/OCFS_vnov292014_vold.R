 ...
 #function to evaluate k-fold CV
		...
		folderror<-{}
			...
			#folderror_cur_perm<-model$mean_acc
				...
				#fitfunc<-((accuracyweight*folderror)*(folderror-folderror_perm))-(featweight*100*(num_feat/length(particle)))
				
				#fitfunc<-((accuracyweight)*(folderror-folderror_perm))-(featweight*100*(num_feat/length(particle)))			

				fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror*0.5)-(featweight*100*(num_feat/length(particle)))	
				}else{

					#fitfunc<-((accuracyweight*folderror)*(folderror-folderror_perm))
					#fitfunc<-(accuracyweight*(folderror-folderror_perm))
					
					fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror*0.5)	
...
performCMA<-function(trainm, testm, outloc, maxnum, minnum, stepitr, gsmethods, percentTest,featweight, accuracyweight, kname, maxitrs, minpresent, norm_method, tolerance=1, classindex, upclasssep, dclasssep, removeindex, numfacts,numfolds=10,evalmethod="CV",CVfoldthresh=0.3,backward.sel=FALSE, scheme_val="one-vs-all",iter_learn=5)
{

data_dim<-dim(trainm)
write.csv(testm[,1], paste(outloc,"orig_40test.csv",sep=""), row.names=FALSE)
write.csv(trainm[,1], paste(outloc,"orig_60train.csv",sep=""), row.names=FALSE)
...
#print(trainm[1:5,c(classindex-1,classindex)])
#meantrainimt=mean(trainm[,classindex])
#print("mean is ")
#print(meantrainimt)
...
#trainm[which(trainm[,classindex]>=upsep),classindex]="A"
#trainm[which(trainm[,classindex]<=dsep),classindex]="B"
#trainmA<-trainm[which(trainm[,classindex]=="A"),]
#trainmB<-trainm[which(trainm[,classindex]=="B"),]

#trainm<-rbind(trainmA,trainmB)
print("dim of trainm is ")
print(dim(trainm))
print(levels(trainm[,classindex]))
#print(trainmB[1:4,1:10])
...
nci_y<-as.factor(trainm[,classindex])
rm(trainm)
...
bootstrap_val=FALSE,
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",constriction_factor=1,select.global.best=TRUE,numnodes=20)
{
...
max_num_feats=round(maxnum*dim(trainm)[2])
				...
				if(agent_behavior[row]==3){
					...
					if(agent_behavior[row]==2){
					  		...
					  		x_curbest_ind<- nn_search_res[i,c(1:num_neighbors)]  #nn_search(i,x) #getnearestneighbor
								...
								x[row,]<-apply(x[x_curbest_ind,],2,function(x){y<-quantile(x,0.75);return(round(y));})
                                                        
							}else{
                                                        	x[row,]<-x[x_curbest_ind[1],]
                                                        }
					ran<-runif(d_dim[2],0,1)
						...
						if(agent_behavior[row]==1){
						
						x[row,]<-x_gbest
						temp_x<-rep(1,length(x_gbest))
						x[row,]<-abs(temp_x-x[row,])
...
print("##################################")
	...
	stopCluster(cl)	
		...
		num_itr_thresh<-round(minselect.pct*globalpso_maxitr)
