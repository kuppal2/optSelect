...
nn_search<-function(particle_ind, part_group)
{
 ...
 #function to evaluate k-fold CV
        eval_fit_kfold_diff<-function(particle, trainm,trainclass,numfolds,errortype="AUC",accuracyweight=5,featweight=0.06,max_num_feats=10,kname="radial")
        {
...
print(paste("accuracyweight: ",accuracyweight,sep=""))
		...
		if(bootstrap_val==TRUE)
			{
				...
				{
				res1<-clusterApply(cl,part_list,eval_fit_kfold_diff,trainm=trainm,trainclass=trainclass,numfolds=numfolds,errortype=errortype,accuracyweight=accuracyweight,featweight=featweightcur,max_num_feats=max_num_feats,kname=kname)
