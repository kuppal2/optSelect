...
svm_acc[i]<-(beracc*100)
	...
	#perm_acc<-as.numeric(perm_acc)
		...
		folderror_test<-foldacc #mean(origtenfoldacc)
		folderror<-(foldacc)
...
#if(FALSE)
{
		#folderror_test<-folderror+runif(1,-5,5)
		 #folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(1*(sd(folderror_perm,na.rm=TRUE))) #/sqrt(num_itrs))
       ...
       # fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror_test)+(featweight*100*((max_num_feats-num_feat)/length(particle)))
	                        }else{
