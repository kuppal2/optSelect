...
featweight=0.01
fitfunc<-accuracyweightA*(folderror-folderror_perm)+accuracyweightB*(folderror_test2)-(100*featweight*((dim(trainset)[2]/dim(trainm)[2])))
folderror_perm<-perm_acc
folderror<-foldacc
...
prob_behavior_particles<-array(0, dim=c(num_part, dim(transition_matrix)[2]))
	for (row in 1:num_part)
	 ...
	 fitness_res<- eval_fit_test_diff(particle=x[1,], numfolds=numfolds,trainm=trainm,trainclass=trainclass,testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,featweight=featweight,accuracyweightA=accuracyweightA,
        accuracyweightB=accuracyweightB,max_num_feats=maxnum)
