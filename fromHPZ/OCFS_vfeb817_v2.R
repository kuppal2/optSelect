...
testset_acc<-svm_acc[i]

#}

set.seed(100)
seed_vec<-sample(1:10000000,10)
beracc<-{}
svm_acc<-{}
for(i in 1:num_itrs){
#perm train
...
train_permacc<-mean(svm_acc,na.rm=TRUE)+1.96*sd(svm_acc,na.rm=TRUE)/(sqrt(num_itrs))
...
fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(testset_acc-train_permacc)+accuracyweightC*(folderror_test2)+accuracyweightD*(folderror)+((featweight*(max((100*(max_num_feats-num_feat))/max_num_feats,100))))
