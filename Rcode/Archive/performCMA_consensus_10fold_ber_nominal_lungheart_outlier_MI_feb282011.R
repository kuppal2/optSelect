...
performCMA<-function(datafile, outloc, maxnum, minnum, stepitr, gsmethods, percentTest,featweight, accuracyweight, kname, maxitrs, minpresent, norm_method, tolerance=1, classindex, upclasssep, dclasssep, removeindex, numfacts)
{
...
varsel <- GeneSelection(nci_xm,nci_y, learningsets = train, method=c("rfe"))
	genelist={}
	for(i in 1:numfolds)
	{
...
for(m in 1:length(gsmethods))
{
	...
	while(num_g<maxnum)
	{
		...
		#foldacc<-min(model$accuracies)
		foldacc<-1*(model$tot.accuracy) #+0.3*(beracc)
