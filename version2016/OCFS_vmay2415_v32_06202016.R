...
write.table(scoringmatrix,file="scoringmatrix.txt",sep="\t")
...
#						accuracyweightB=accuracyweightB,featweight=featweightcur,maxnum=maxnum)
			...
			if(min_fit_x < (fitness_gbest) && (no_change<maxitrreset))
			{
			
				fit_diff_val<-abs(min_fit_x)-abs(fitness_gbest)
				if(fit_diff_val>0.5){
		...
		print(agent_behavior)
			...
			d1pct<-100*(d1/length(x_lbest_mean))
				...
				if(k>global_max_itr)
				{
						...
						if(fitness_gbest<(-minfitnessthresh) & (global_no_change>=global_max_itr)){ #& d1pct<=50){
