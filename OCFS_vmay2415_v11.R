...
svm_acc[i]<-(beracc*100)
	...
	#perm_acc<-as.numeric(perm_acc)
		...
		foldacc<-min(origtenfoldacc,na.rm=TRUE) #-sd(origtenfoldacc,na.rm=TRUE)
		folderror_test<-mean(origtenfoldacc,na.rm=TRUE)
		folderror<-(foldacc)
...
#if(FALSE)
{
		#folderror_test<-folderror+runif(1,-5,5)
		 folderror_perm<-max(folderror_perm,na.rm=TRUE) #+(1*(sd(folderror_perm,na.rm=TRUE))) #/sqrt(num_itrs))
