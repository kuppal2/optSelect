...
performPSO<-function(trainm, testm, outloc, numitrs, reinititrs, numpart, c1, c2, kname, percentTest, featweight, accuracyweight)
{
	...
	eval_fit_kfold<-function(particle, k)
	{
		...
		if(num_feat>1)
		{
			...
			#model<-svm(trainset, trainclass, type="C", kernel=kname, degree=tune_degree, gamma=tune_gamma, cost=tune_cost, cross=kcross)
			 ...
			 model<-svm(as.factor(trainset$Carotid.IMT)~., data=trainset, type="C", cross=10)
	...
	model_train_err<-svm(as.factor(modtrain$Carotid.IMT)~., data=modtrain, type="C", cross=10)

	model_train_valid<-svm(as.factor(modtrain$Carotid.IMT)~., data=modtrain, type="C", cross=10)
