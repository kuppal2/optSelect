...
for(m in 1:length(gsmethods))
{
	...
	print(paste(gsmethods[m],":",length(bestgenelist), ":", bestmod," : ",num_g,sep=""))	
		...
		if(varselmethod=="forward"){
            ...
            maxnum=length(genelist) #min(length(genelist),maxnum)
...
if(maxnum>1){
	...
	constriction_factor=0.792
    
    #constriction_factor=10
}
...
boostweight=boostweight,train.pct=train.pct,...)
        ...
        median_selection_mat<-apply(scoringmatrix,1,median)
...
print(ki_res)
        ...
        bestgenelist=median_selection_mat
...
#						accuracyweightB=accuracyweightB,featweight=featweightcur,maxnum=maxnum)
			...
			for (i in 1:num_part)
			{
				...
				social_status<-1
					...
					if(best_action==1){
                        ...
                        third_quartile_fitness<-quantile(fitness_x,na.rm=TRUE,0.5)
                        top_fitness_neighbors<-which(fitness_x>=third_quartile_fitness)
                        
                        #using summary of neighbors in v10
                        x_curbest_popsummary<-apply(x[top_fitness_neighbors,],2,function(x){y<-quantile(x,0.5);return(round(y));})
			...
			r3<-runif(dimsize,0,1)
                ...
                for (j in 1:dimsize)
				{
					...
					v[i,j]<-constriction_factor*((w*v[i,j])+(c1*r1[j]*(x_lbest[i,j]-x[i,j]))+(c2*r2[j]*(x_curbest[j]-x[i,j])*social_status)+(0.5*r3[j]*(x_curbest_popsummary[j]-x[i,j])*social_status))
			 ...
			 x_lbest_mean<-apply(x_lbest,2,median)
