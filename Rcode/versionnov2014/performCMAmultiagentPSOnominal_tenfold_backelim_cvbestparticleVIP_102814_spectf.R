...
outloc<-"/home/stu/kuppal3/Research/SpectF_data/Results/itr1pso100wrand_neigh0.6conf0.1behav0.3_transmatBER_minsel0.5_CV_rep50/"
		...
		repeat
		{
			...
			for (i in 1:num_part)
			{
				...
				if(k%%behavior_reset_itr==0){
					...
					if(rank[i]>=rank_sum_vec[5]){

						prob_behavior_particles[i,]<-c(0.1,0.4,0.5,0)
					}else{

						if(rank[i]>=rank_sum_vec[3]){

                                                	prob_behavior_particles[i,]<-c(0.3,0.4,0.3,0)
                                        		}else{
