...
for(m in 1:length(gsmethods))
{
	...
	while(num_g>minnum)
	{
		...
		trainclass<-nci_y
       		trainset1<-cbind(trainclass,trainset)
		...
		fitfunc_all<-{}
		 seed_vec<-c(129532,839147,407700)
				...
				tstat<-qt(c(.975), df=length(folderror_perm)-1)
                                folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(tstat*(sd(folderror_perm,na.rm=TRUE)/sqrt(3)))
                                fitfunc<-(100*(tenfoldacc-folderror_perm))+accuracyweight*(tenfoldacc)-(featweight*100*(length(col_sel)/dim(nci_xm)[2])) #(featweight*(num_feat))
