...
source("/home/stu/kuppal3/Research/Feature_selection/performCMA_consensus_tenfold_ber_nominal_lungheart_train_test_sep192012_backward.R")
...
#/home/stu/kuppal3/Research/Feature_selection/Datasets/Golub
#trainy<-read.csv("/home/stu/kuppal3/Research/Feature_selection/Datasets/ARCENE/arcene_train.labels", header=FALSE)
#testy<-read.csv("/home/stu/kuppal3/Research/Feature_selection/Datasets/ARCENE/arcene_valid.labels", header=FALSE)

#traind<-read.table("/home/stu/kuppal3/Research/Feature_selection/Datasets/ARCENE/arcene_train.data", sep=" ",header=FALSE)
#testd<-read.table("/home/stu/kuppal3/Research/Feature_selection/Datasets/ARCENE/arcene_valid.data", sep=" ",header=FALSE)

#trainy<-read.csv("/home/stu/kuppal3/Research/Feature_selection/Datasets/Golub/golub.train.labels.txt", header=TRUE)
#testy<-read.csv("/home/stu/kuppal3/Research/Feature_selection/Datasets/Golub/golub.test.labels.txt", header=TRUE)

#traind<-read.table("/home/stu/kuppal3/Research/Feature_selection/Datasets/Golub/golub.train.data.txt", sep="\t",header=TRUE)
#testd<-read.table("/home/stu/kuppal3/Research/Feature_selection/Datasets/Golub/golub.test.data.txt", sep="\t",header=TRUE)
...
classB=datafile[which(datafile[,data_dim]>=10),]

classA[,data_dim]="Zero"
classB[,data_dim]="TenorMore"
...
parentoutloc<-"/home/stu/kuppal3/Research/Feature_selection/Datasets/LungCancer/"
#itr1_backwardmultiagent/"
...
trnames<-colnames(traind)
tenames<-colnames(testd)
m1<-match(trnames,tenames)
...
mingenes=1
stepitr=1
methods=c("kruskal.test", "f.test", "rfe", "rf","lasso", "elasticnet", "wilcox.test", "welch.test")
#methods=c("kruskal.test", "f.test", "rfe", "wilcox.test", "welch.test", "t.test")
#methods=c("t.test")
percentTest=0.40
kname="radial"
maxitrs=100
...
outloc<-paste("itr1minpres",minpresent,"normmeth",norm_method,"itr",itr,sep="")
dir.create(outloc,showWarnings=FALSE)

#outloc<-paste(parentoutloc,"/",outloc,sep="")
setwd(outloc)
...
CMAres<-performCMA(traind, testd, outloc, maxfeatspercent, minfeats, stepitr, methods, percentTest, featWeight,accWeight, kname, maxitrs, minpresent, norm_method, tolerance, classindex, upclasssep, dclasssep, removeindex, maxfacts)
	...
	eval_fit_kfold_diff<-function(particle)
	{
		...
		if(num_feat>1)
		{
			...
			fitval<-predict(model, trainset[,-c(1)])
                        ...
                        folderror_cur<-0.7*(model$tot.accuracy+(0.3*(beracc)*100))
			folderror<-c(folderror,folderror_cur) #model$tot.accuracy)

                        }
                        folderror<-mean(folderror)-sd(folderror)
	...
	for (k in 1:itr)
		{
			...
			for (i in 1:num_part)
			{
				...
				agent_behavior<-runif(1,0,1)

				x_curbest<-x_gbest
                                        if(agent_behavior<confusionprob)
                                        {
                                                        ran<-runif(d_dim[2])
                ...
                {                               
                        ...
                        if (ran[col]<0)         
                        {
                                ...
                                x_curbest[col]<-1
                                                ...
                                                if(agent_behavior<followerprob){
                                                        x_curbest_ind<- nn_search(i,x) #getnearestneighbor
                                                	x_curbest<-x[x_curbest_ind,]
						}else{
							x_curbest<-x_gbest
						}
