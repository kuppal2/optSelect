 ...
 #function to evaluate k-fold CV
		...
		folderror<-{}
			...
			#folderror_cur_perm<-model$mean_acc
				...
				fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror)-(featweight*100*(num_feat/length(particle)))
		...
		if(bootstrap_val==TRUE)
			...
			else
			{
				...
				if(no_change>maxitrreset)
				{
					...
					if(global_no_change>global_max_itr & length(which(x_lbest>0))>minnum){
			...
			agent_behavior<-sample(x=c(1,2,3),prob=c(0.5,0.25,0.25), size=num_part,replace=TRUE)	
