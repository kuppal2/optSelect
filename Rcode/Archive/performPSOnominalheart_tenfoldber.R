###################################################################
performPSO<-function(trainm, testm, trainclass, testclass, outloc, numitrs, reinititrs, numpart, c1, c2, kname, percentTest, featweight, accuracyweight)
{	
	...
	eval_fit_kfold<-function(particle, k)
	{
		...
		if(num_feat>1)
		{
			...
			folderror<-0.5*(model$tot.accuracy+0.5*(beracc))
			#folderror<-(1-ber)*100
			rm(trainset)
