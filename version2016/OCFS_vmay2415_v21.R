...
svm_acc[i]<-(beracc*100)
	...
	#function to evaluate k-fold CV
	eval_fit_test_diff<-function(particle, numfolds,trainm,trainclass,testm,testclass,errortype="AUC",kname="radial",weightA=0.5,
	weightB=0.2,weightC=0.05,weightD=0.2,weightE=0.05,max_num_feats=10)
	{
...
fitfunc<-(weightA*(folderror-folderror_perm))+weightB*(folderror_train-folderror_train_perm)+weightC*(folderror_test2)+weightD*(folderror)+((weightE*(max((100*(max_num_feats-num_feat))/max_num_feats,100))))
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,evalFunc,boostweight=NA,pct_train=0.8,...)
{
...
print(paste("weightE: ",weightE,sep=""))
...
boostweight=boostweight,pct_train=pct_train,...)
...
print(ki_res)
		...
		best_fold<-which(testacc>=summary_testacc[1])


		print(best_fold)
		if(length(best_fold)>1){
		sm1<-scoringmatrix[,best_fold]
			sm2<-apply(sm1,1,median) #as long as 1 is selected
			bestgenelist<-which(sm2>=1)
		}else{
		sm1<-scoringmatrix[,best_fold]
		bestgenelist<-which(sm1==1)
		}
...
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,seednum=NA,varnames=NA, itr.terminate=TRUE,evalFunc,trainm=NA,trainclass=NA,boostweight=NA,pct_train=0.8,...)
{
...
numtrain<-(pct_train*nrow(trainm))
evalmethod='MCCV'
set.seed(321)
print("s")
print(numtrain)
...
prob_behavior_particles<-array(0, dim=c(num_part, dim(transition_matrix)[2]))
	for (row in 1:1)
	 ...
	 fitness_res<- eval_fit_test_diff(particle=x[1,], numfolds=numfolds,trainm=trainm,trainclass=trainclass,testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,weightA=weightA,weightB=weightB,weightC=weightC,weightD=weightD,weightE=weightE,max_num_feats=maxnum)

	cv_res<-fitness_res$cverror
	cvperm_res<-fitness_res$cvpermerror
	fitness_res<-fitness_res$fitfunc

	print("Starting fitness")
	print(fitness_res)

	fitnress_res<-100000000000000
	  fitness_x<-array(fitness_res, dim=c(num_part, 1))
         fitness_gbest<-fitness_res
        cverror_gbest<-(cv_res)
        cvpermerror_gbest<-(cvperm_res)
        prev_gbest<-fitness_res

         fitness_lbest<-array(fitness_res, dim=c(num_part, 1))
    ...
    agent_behavior <- sample(x=1:dim(transition_matrix)[1], size = num_part, replace = TRUE, prob = c(confusionprob, followerprob,leaderprob,(1-(confusionprob+followerprob+leaderprob))))
				...
				featweightcur<-weightE #featweight.max-(((featweight.max-featweight.min)/itr)*k)
	...
	# fitness_res<- eval_fit_test_diff(particle=x[1,], numfolds=numfolds,trainm=trainm,trainclass=trainclass,testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,featweight=featweight,weightA=weightA,weightB=weightB,weightC=weightC,weightD=weightD,weightE=weightE,max_num_feats=maxnum)
	
				res1<-clusterApply(cl,part_list,evalFunc,numfolds=numfolds,trainm=subtrain,trainclass=subtrainclass,
						testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,weightA=weightA,weightB=weightB,weightC=weightC,weightD=weightD,weightE=weightE,maxnum=maxnum)
				...
				else
				{
					...
					#validation set
                                res1<-clusterApply(cl,part_list,eval_fit_test_diff,numfolds=numfolds,trainm=trainm,trainclass=trainclass,
                                                testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,weightA=weightA,weightB=weightB,weightC=weightC,weightD=weightD,weightE=weightE,max_num_feats=maxnum)
			...
			else
			{
				...
				if(no_change>maxitrreset)
				{
					...
					d1<-dist(as.matrix(rbind(x_lbest_vec,overall_x_gbest)))^2
						...
						if(d1pct<=2 && global_no_change>(global_max_itr*1.5))
						{	
					#	print(dim(x_lbest))	
