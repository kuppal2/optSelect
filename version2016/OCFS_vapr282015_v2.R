 ...
 #function to evaluate k-fold CV
	...
	accuracyweightB=1,max_num_feats=10)
	{
		...
		if(num_feat>1)
		{
			...
			folderror_reg<-c(folderror_reg,folderror_cur_reg)
...
svm_acc[i]<-(beracc*100)
	...
	folderror<-svm_acc[i]
tstat<-qt(c(.975), df=length(folderror_reg)-1)
                folderror_reg<-mean(folderror_reg,na.rm=TRUE)-(tstat*(sd(folderror_reg,na.rm=TRUE)/sqrt(3)))
                                folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(tstat*(sd(folderror_perm,na.rm=TRUE)/sqrt(3)))
                                if(num_feat>max_num_feats){
                                fitfunc<-(accuracyweightA*(folderror_reg-folderror_perm))+accuracyweightB*(folderror)-(featweight*100*(num_feat/length(particle))) #(featweight*(num_feat))
                                }else{
...
iter_learn<-numfolds
train<-GenerateLearningsets(y=nci_y, method="MCCV", fold=numfolds, strat=FALSE, ntrain=(0.8*length(nci_y)))
}
#set.seed(321)
...
if(maxnum<minnum || maxnum<1){

	maxnum=data_dim[2]
}
maxnum=max(c(10,maxnum),na.rm=TRUE)

maxnum=min(data_dim[2],10)
print("maxnum is ")
print(maxnum)
...
for(m in 1:length(gsmethods))
{
	...
	while(num_g>minnum)
	{
		...
		print(names(train))
		trainlearningsets<-train@learnmatrix
			for(pitr in 1:dim(train@learnmatrix)[1])
		        {
		
				trainm<-nci_xm[-c(trainlearningsets[pitr,]),col_sel]
         trainclass<-nci_y[-c(trainlearningsets[pitr,])]
        subtest<-nci_xm[c(trainlearningsets[pitr,]),col_sel]
        subtestclass<-nci_y[c(trainlearningsets[pitr,])]

				trainset1<-cbind(trainclass,trainm)

                        trainset1<-data.frame(trainset1)
			...
			folderror_perm<-{}
                        ...
                        setseed=seed_vec[f]
                        model<-svm_cv(v=numfolds,x=trainset1[,-1],y=trainset1$trainclass,kname=kname,errortype=errortype,setseed=setseed)
                        folderror_cur_reg<-model$confint[1]
			folderror_reg<-c(folderror_reg, folderror_cur_reg)
                        #folderror_cur<-model$mean_acc
                        #print(model)
                        ...
                        #folderror<-c(folderror,folderror_cur) #model$tot.accuracy)
                        set.seed(seed_vec[f])
                        rand_ind<-sample(x=seq(1,length(trainset1$trainclass)),size=length(trainset1$trainclass))
                        model<-svm_cv(v=numfolds,x=trainset1[,-1],y=trainset1$trainclass[rand_ind],kname=kname,errortype=errortype,setseed=setseed)
                        folderror_cur_perm<-model$confint[2]
                        folderror_perm<-c(folderror_perm,folderror_cur_perm)
                                                rm(model)
                        }
...
if(FALSE){
                     testset<-subtest
                mod_cv <- svm(x=trainset1[,-1],y=trainset1$trainclass, type="C",kernel=kname)
predfit<-predict(mod_cv,testset)
...
for(c in 1:dim(svm_table)[1]){
testclass_ind<-which(subtestclass==class_names[c])
beracc<-c(beracc,length(which(predfit[testclass_ind]==subtestclass[testclass_ind]))/length(testclass_ind))
}
print(beracc)
...
folderror<-beracc
		...
		print(folderror)
                ...
                folderror_reg<-mean(folderror_reg,na.rm=TRUE)-(tstat*(sd(folderror_reg,na.rm=TRUE)/sqrt(3)))
                                folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(tstat*(sd(folderror_perm,na.rm=TRUE)/sqrt(3)))
                                fitfunc<-(100*(folderror_reg-folderror_perm))+accuracyweight*(folderror)-(featweight*100*(length(col_sel)/dim(nci_xm)[2])) #(featweight*(num_feat))

				fitfunc_all<-c(fitfunc_all,fitfunc)
				}

		fitfunc_all<-mean(fitfunc_all,na.rm=TRUE)
		#fitfunc<-accuracyweight*(foldacc) #+featweight*(1-num_g/dim(nci_xm)[2])
		if(fitfunc_all>bestmod)
                {
                        bestmod<-fitfunc
...
constriction_factor=constriction_factor,select.global.best=select.global.best,numnodes=numnodes,bootstrap.itr=bootstrap.itr,evalFunc=evalFunc,trainm=trainm,trainclass=trainclass,...)
		...
		s1<-summary(summat)
		num_itr_thresh<-round(s1[3]) #ceiling(minselect.pct*globalpso_maxitr)
