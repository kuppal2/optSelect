...
svm_acc[i]<-(beracc*100)
		...
		folderror<-svm_acc[i]
				...
				folderror_cur_reg<-mean(folderror_cur_reg,na.rm=TRUE)
...
set.seed(seednum)
trainlearningsets<-GenerateLearningsets(y=trainclass, method=evalmethod, fold=numfolds, strat=TRUE, niter=10,ntrain=numtrain)
trainlearningsets<-trainlearningsets@learnmatrix
globalpso_maxitr=dim(trainlearningsets)[1]
