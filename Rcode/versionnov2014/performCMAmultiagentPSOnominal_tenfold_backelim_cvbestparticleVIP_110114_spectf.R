...
outloc<-"/home/stu/kuppal3/Research/SpectF_data/Results/itr1CMApso100wlinincbehav5itr_transmatminsel0.7_10foldBERminpres1change30/"
		...
		if(bootstrap_val==TRUE)
			...
			r3<-runif(d_dim[2],0,1)
			       ...
			       for (j in 1:d_dim[2])
				{
					...
					v[i,j]<-((w*v[i,j])+(c1*r1[1]*(x_lbest[i,j]-x[i,j]))+(c2*r2[1]*(x_curbest[j]-x[i,j])))
		...
		#	print(head(x_lbest))
			...
			#	}
				...
				if(k>itr){
						...
						print("k")
						if(fitness_gbest<(-80) & (global_no_change<=maxitr)){
