...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,evalFunc,percentTrain=0.7,...)
{
...
constriction_factor=constriction_factor,select.global.best=select.global.best,numnodes=numnodes,bootstrap.itr=bootstrap.itr,evalFunc=evalFunc,trainm=trainm,trainclass=trainclass,percentTrain=0.7,...)
		...
		KI_index<-get_KIindex(scoringmatrix)
	
		if(KI_index<(1)){
		
			num_itr_thresh<-median(summat)
			if(num_itr_thresh<2){
...
return(list(scoringmatrix=scoringmatrix,bestfeatlist=bestgenelist,bestfeatnames=feat_names,model=mod,psotrainmatrix=trainm,psotestmatrix=testm))
...
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,seednum=NA,varnames=NA, itr.terminate=TRUE,percentTrain=0.8,evalFunc,trainm=NA,trainclass=NA,...)
{
...
numtrain<-round(percentTrain*nrow(trainm))
evalmethod='MCCV'
set.seed(321)
print("s")
print(numtrain)
