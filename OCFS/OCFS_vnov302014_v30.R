...
nn_search<-function(particle_ind, part_group)
{
 ...
 #function to evaluate k-fold CV
	...
	accuracyweightB=1,max_num_feats=10,seednum=NA)
	{
		...
		if(num_feat>1)
		{
			...
			#folderror_cur_reg<-model$mean_acc[1]
			#print(model)
			#print(folderror_cur)	
			#folderror<-c(folderror,folderror_cur) #model$tot.accuracy)
			#set.seed(seed_vec[f])
			...
			ntest<-floor(dim(trainset)[1]/numfolds)
...
svm_acc[i]<-(beracc*100)
		...
		folderror<-svm_acc[i]
                                ...
                                #folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(1.96*(sd(folderror_perm,na.rm=TRUE)/sqrt(2)))
        			folderror_perm<-max(folderror_perm,na.rm=TRUE)
				#folderror_perm<-folderror_perm+(1.96*sqrt((folderror_perm*(1-folderror_perm))/ntest))
			
				folderror_cur_reg<-min(folderror_cur_reg,na.rm=TRUE)
				#folderror_cur_reg<-folderror_cur_reg-(1.96*sqrt((folderror_cur_reg*(1-folderror_cur_reg))/ntest))
...
rand_ind<-sample(x=1:length(tempclass),size=length(tempclass))
...
n=length(svm_acc)
talpha_2<-qt(conflevel*0.01,df=n-1)
error <- (talpha_2)*(sd_acc/sqrt(length(svm_acc)))
...
evalMode="CV1",
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",percentTest=0.4,
...
maxnum=maxnum,minnum=minnum,inertia_method=inertia_method,particlebehav_method=particlebehav_method,percentTest=percentTest,
...
numtrain<-(0.7*nrow(trainm))
evalmethod='MCCV'
set.seed(seednum)
trainlearningsets<-GenerateLearningsets(y=trainclass, method=evalmethod, fold=numfolds, strat=TRUE, niter=numfolds,ntrain=numtrain)
...
globalpso_maxitr=dim(trainlearningsets)[1]
		...
		setsize<-min(dim(alltrainm)[1],100)
		nlevels<-table(as.factor(trainclass))
				 if(evalMode=="CV1")
	{
	...
	subtestclass<-alltrainclass[c(trainlearningsets[pitr,])]
		set.seed(seednum)
		subtrain_ind<-sample(x=seq(1,dim(trainm)[1]),size=setsize,replace=TRUE)
                                trainm<-trainm[subtrain_ind,]
                               trainclass<-trainclass[subtrain_ind]
		set.seed(seednum)
		subtest_ind<-sample(x=seq(1,dim(subtest)[1]),size=setsize,replace=TRUE)
                                subtest<-subtest[subtest_ind,]
                               subtestclass<-subtestclass[subtest_ind]	
	}else{
		if(evalMode=="CV2"){
...
svm_acc[i]<-(beracc*100)
	...
	#	print(folderror)
				...
				folderrorkfold<-mean(folderrorkfold,na.rm=TRUE) #-(1.96*(sd(folderrorkfold,na.rm=TRUE)/sqrt(2)))
                                folderror_perm<-mean(folderror_perm,na.rm=TRUE) #+(1.96*(sd(folderror_perm,na.rm=TRUE)/sqrt(2)))
                                if(num_feat>max_num_feats){
...
if(is.na(seednum)==TRUE){
	seednum=round(runif(1,1,999999))
}
		if(evalMode=="CV1" || evalMode=="CV2" || evalMode=="bootstrap")
{

numtrain<-(percentTest*nrow(trainm))
evalmethod='MCCV'
set.seed(seednum)
print("s")
print(numtrain)
...
set.seed(seednum)
trainlearningsets<-GenerateLearningsets(y=trainclass, method=evalmethod, fold=numfolds, strat=FALSE, niter=10,ntrain=numtrain)
trainlearningsets<-trainlearningsets@learnmatrix
globalpso_maxitr=dim(trainlearningsets)[1]
...
print("here")
  scoringmatrix=matrix(0,dimsize,globalpso_maxitr)
	...
	{		
		...
		setsize=max(dim(alltrainm)[1],100)
	...
	featweightcur<-featweight.min+(((featweight.max-featweight.min)/k))				
		...
		if(evalMode=="bootstrap")
			{
				...
				#subtrain_ind<-sample(x=seq(1,dim(trainm)[1]),size=dim(alltrainm)[1]*percentTest,replace=FALSE)
				...
				subtrain<-trainm
				subtrainclass<-trainclass
				subtrain_ind<-sample(x=seq(1,dim(subtrain)[1]),size=dim(subtrain)[1],replace=TRUE)
				
				subtrain<-subtrain[subtrain_ind,]
                                subtrainclass<-subtrainclass[subtrain_ind]
	 ...
	 #agent_behavior <- sample(x=1:dim(transition_matrix)[1], size = num_part, replace = TRUE, prob = c(confusionprob, followerprob,leaderprob,(1-(confusionprob+followerprob+leaderprob))))
		...
		print(agent_behavior)
					...
					agent_behavior[row]<-sample(x=seq(1,dim(transition_matrix)[1]),prob=c(0.7,0.1,0.19,0.01),size=num_part,replace=TRUE)
			...
			for (i in 1:num_part)
			{
				...
				social_status<-1
					...
					if(best_action==1){
							...
							#x_curbest<-apply(x,2,median)


							x_curbest<-apply(x,2,function(x){y<-quantile(x,0.75);return(round(y));})
						}else
						{
							if(best_action==2){
			...
			r3<-runif(dimsize,0,1)
			       ...
			       for (j in 1:dimsize)
				{
					...
					beta_prior<-(length(which(x_lbest[,j]==1))/length(x_lbest[,j]))	
					...
					#if(S>=beta_prior)
					{
							...
							#count_feat_sel[j]<-count_feat_sel[j]+1
					}			
					else
					{
						x[i,j]<-0
