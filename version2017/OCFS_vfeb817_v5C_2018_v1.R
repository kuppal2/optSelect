							...
							for (row in 1:num_part)
                            {
                                ...
                                for (col in 1:dimsize)
                                {
                                        ...
                                        if (rand_num<0.33)
                                        {
                                            ...
                                            if (rand_num>=0.33 & rand_num<0.667){
                                                    if(x1sum[col]>0.5){x[row,col]<-1}else{x[row,col]<-0}
                                            }else{
                                                x[row,]<-rev(x_gbest)
                                            }
		...
		print(agent_behavior)
            ...
            r5<-runif(dimsize,0,1)
			       ...
			       for (j in 1:dimsize)
				{
                    ...
                    v[i,j]<-constriction_factor*((w*v[i,j])+(c1*r1[j]*(x_lbest[i,j]-x[i,j]))+
                    (c2*r2[j]*(x_curbest[j]-x[i,j])*social_status)+(c2*r3[j]*(x_curbest_popsummary[j]-x[i,j])*social_status)
                    +(c2*r4[j]*(x1sum[j]-x[i,j])*social_status))
