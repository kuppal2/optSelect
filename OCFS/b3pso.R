...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,trainlearningsets=NA,evalFunc,varnames=NA, itr.terminate=TRUE,...)
{
		...
		if(evalMode=="bootstrap")
			{
				...
				res1<-clusterApply(cl,part_list,evalFunc,numfolds=numfolds,trainm=subtrain,trainclass=subtrainclass,
						testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,accuracyweightA=accuracyweightA,accuracyweightB=accuracyweightB,featweight=featweightcur,maxnum=maxnum)
				...
				else
				{
					...
					#validation set
                             #   res1<-clusterApply(cl,part_list,evalFunc,numfolds=numfolds,trainm=trainm,trainclass=trainclass,
				...
				#		accuracyweightB=accuracyweightB,featweight=featweightcur,max_num_feats=maxnum)
				   ...
				   res1<-clusterApply(cl,part_list,evalFunc,numfolds=numfolds,errortype=errortype,kname=kname,accuracyweightA=accuracyweightA,
						accuracyweightB=accuracyweightB,featweight=featweightcur,max_num_feats=maxnum,...)		
                                 for(np in 1:num_part){
                                        #fitness_x<-sapply(res1,head)
					...
					#print(dim(trainm))
				res1<-clusterApply(cl,part_list,evalFunc,numfolds=numfolds,errortype=errortype,accuracyweightA=accuracyweightA,
				accuracyweightB=accuracyweightB,featweight=featweightcur,max_num_feats=maxnum,kname=kname,...) #trainm=trainm,trainclass=trainclass,numfolds=numfolds,errortype=errortype,accuracyweightA=accuracyweightA,accuracyweightB=accuracyweightB,featweight=featweightcur,maxnum=maxnum,kname=kname)
