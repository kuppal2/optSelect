...
outloc<-"/Users/karanuppal/Documents/Gatech/Projects/Algorithms/TwostagePSO/Rcode/versionnov2014//itr1CMAPSO100itrsCVpres1change5max30w1fit50v8tryfeatw1000lindec/"
...
nn_search<-function(particle_ind, part_group)
{
 ...
 #function to evaluate k-fold CV
        eval_fit_kfold_diff<-function(particle, numfolds,errortype="AUC",featweight=0.06)
        {
...
prob_behavior_particles<-array(0, dim=c(num_part, dim(transition_matrix)[2]))
	...
	#for (k in 1:itr)
		repeat
		{
			...
			for (i in 1:num_part)
			{
				...
				featweightcur<-featweight.max-(((featweight.max-featweight.min)/maxitr)*k)
		...
		if(bootstrap_val==TRUE)
			{
				...
				{
				res1<-clusterApply(cl,part_list,eval_fit_kfold_diff,numfolds=numfolds,errortype=errortype,featweight=featweightcur)
                                fitness_x<-sapply(res1,head)	
	...
	write.table(itr_data, file=filestr, sep="\t", row.names=F)
		...
		if(length(feat_ind)<2 | is.na(minselect.pct)==TRUE)
		{
