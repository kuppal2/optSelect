...
for(m in 1:length(gsmethods))
{
	...
	while(num_g<=maxnum)
		...
		maxnum=min(length(genelist),maxnum)
			...
			print(paste(gsmethods[m],":",maxnum, ":", fitfunc,sep=""))
...
scoringmatrix_temp<-scoringmatrix[,-c(bad_methods)]
}else{
    
    scoringmatrix_temp<-scoringmatrix
}

for(i in 1:dim(scoringmatrix_temp)[2]){if(length(which(m1[,i]==1))>0){m2[[i]]<-paste("var",which(m1[,i]==1),sep="")}}
print(m2)
m3<-as.data.frame(m2)
m4<-t(m3)
