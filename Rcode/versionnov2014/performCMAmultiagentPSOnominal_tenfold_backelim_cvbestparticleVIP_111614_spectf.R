...
outloc<-"/Users/karanuppal/Documents/Gatech/Projects/Algorithms/TwostagePSO/Rcode/versionnov2014//itr1CMAPSO100itrspres1change5w1/"
		...
		repeat
		{
			...
			for (i in 1:num_part)
			{
				...
				{	
					...
					print("prob _behavior")
                                        print(i)
                                        print(prob_behavior_particles[i,])
					print(best_action)				
			...
			else
			{
				...
				#	print("exited")
					...
					for (row in 1:num_part)
					{
						...
						prob_behavior_particles[row,]<-c(0.3,0.3,0.3,0.1)	
						ran<-runif(d_dim[2],0,1)
						for (col in 1:d_dim[2])
		...
		#	print(head(x_lbest))
			...
			#	}
				...
				if(k>itr){
						...
						print("k")
						if(fitness_gbest<(-minfitnessthresh) & (global_no_change<=maxitr)){
						...
						break;
							...
							if(reinit_count>3){
								...
								print(paste("Fitness threshold not reached after ",reinit_count," tris! Exiting PSO.",sep=""))
								break;
							}
							print(paste("Fitness threshold not reached after ",k," iterations! Continuing PSO.",sep=""))
							itr=k+itr-1
