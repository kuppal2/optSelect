...
nn_search<-function(particle_ind, part_group)
{
 ...
 #function to evaluate k-fold CV
	...
	#function to evaluate k-fold CV
	eval_fit_test_diff<-function(particle, numfolds,trainm,trainclass,testm,testclass,errortype="AUC",kname="radial",featweight=0.05,accuracyweightA=5,accuracyweightB=1,max_num_feats=10)
	{
...
performCMA<-function(trainm, trainclass, testm, testclass, outloc, maxnum, minnum, stepitr, gsmethods, percentTest,featweight, accuracyweight, kname, maxitrs, minpresent, norm_method, tolerance=1, classindex, numfacts,numfolds=10,evalmethod="CV",CVfoldthresh=0.3,backward.sel=FALSE, scheme_val="one-vs-all",iter_learn=5)
{
...
return(list(modgenelist=bestgenelist, modtraindata=modtrain, modtestdata=modtest, blindtest=testacc, modtrainclass=nci_y, modtestclass=test_y, numfeat=dim(modtrain)[2], testacc=testacc, tenfoldacc=model_train$tot.accuracy))
...
####function: svm_cv
svm_cv<-function(v,x,y,kname="radial",errortype="CV",conflevel=95){
...
run_pso<-function(trainm,trainclass,testm,testclass,transition_matrix,c1=2.05,
c2=2.05,
itr=10,
globalpso_maxitr=1,
global_max_itr=3,
...
bootstrap_val=FALSE,
minfitnessthresh=50,
maxnum=0.5,minnum=3,inertia_method="rankbased",particlebehav_method="rankbased",constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10)
{
...
print(paste("bootstrap_val: ",bootstrap_val,sep=""))
...
pred<-predict(mod,testm)
test.table=table(pred,testclass)
...
print("Test acc is ")
print(testacc)
pred_acc<-multiclass.roc(testclass,as.numeric(pred))
        pred_acc_orig<-pred_acc$auc[1]
        auc_acc<-pred_acc_orig

print("Test AUC:")
print(auc_acc)
featweightcur=featweight.max
if(bootstrap_val==FALSE){
particle<-rep(1,dim(trainm)[2])
fitness_res<-eval_fit_kfold_diff(particle=particle,trainm=trainm,trainclass=trainclass,numfolds=numfolds,errortype=errortype,accuracyweightA=accuracyweightA,accuracyweightB=accuracyweightB,featweight=featweightcur,max_num_feats=max_num_feats,kname=kname)


}else{
particle<-rep(1,dim(trainm)[2])
fitness_res<-eval_fit_test_diff(particle=particle,numfolds=numfolds,trainm=subtrain,trainclass=subtrainclass,
                                                testm=subtest,testclass=subtestclass,errortype=errortype,kname=kname,accuracyweightA=accuracyweightA,accuracyweightB=accuracyweightB,featweight=featweightcur,max_num_feats=max_num_feats)


}

fitness_x<-(-1)*(fitness_res$fitfunc)
fitness_gbest<-fitness_x
...
prob_behavior_particles<-array(0, dim=c(num_part, dim(transition_matrix)[2]))
	...
	featweightcur<-featweight.min+(((featweight.max-featweight.min)/k))				
		...
		if(bootstrap_val==TRUE)
			{
				...
				{
				#lapply(1:bootstrap.itr,function(boot_itr){
				...
				print(table(subtrainclass))
				print(table(subtestclass))
				fitness_x<-{}
				cverror<-{}
				cvpermerror<-{}
				print(dim(subtrain))
			...
			print(boot_fitness)
				...
				{
				res1<-clusterApply(cl,part_list,eval_fit_kfold_diff,trainm=trainm,trainclass=trainclass,numfolds=numfolds,errortype=errortype,accuracyweightA=accuracyweightA,accuracyweightB=accuracyweightB,featweight=featweightcur,max_num_feats=max_num_feats,kname=kname)
			...
			print("Global No change")
				...
				if(no_change>maxitrreset)
				{
					...
					feat_global_num<-length(which(overall_x_gbest==1))
						if(d1pct<=5 && global_no_change>round(global_max_itr)) # && feat_global_num<=max_num_feats)
						#if(global_no_change>round(global_max_itr*1.5))
						{	
						print(paste("d1pct: ",d1pct,sep=""))
						print(length(which(x_lbest_vec==1)))
						print(feat_global_num)
						print(paste("No change for ",global_no_change," iterations. Exiting PSO.",sep=""))
						break;
						}else{
								if(global_no_change>round(global_max_itr) && (global_no_change<round(global_max_itr)*2))
                                                	{
                                               			 print(paste("d1pct: ",d1pct,sep=""))
                                               			 print(length(which(x_lbest_vec==1)))
                                                		...
                                                		#print(paste("No change for ",global_no_change," iterations. Exiting PSO.",sep=""))
                                               			 #break;
								#agent_behavior <- sample(x=1:dim(transition_matrix)[1], size = num_part, replace = TRUE, prob = c(confusionprob, followerprob,leaderprob,(1-(confusionprob+followerprob+leaderprob))))
								#agent_behavior<-rep(3,num_part)
								rand_init_thresh<-0.99
                                                	
									
							}else{

								if(global_no_change>=round(global_max_itr)*2){
		...
		#			v1<-sample(size=num_part*d_dim[2],x=c(0,1),replace=TRUE,prob=c(0.4,0.6))
		#			dim(v1)<-c(num_part, d_dim[2])	
	 ...
	 rand_num<-runif(num_part,0,1)
		...
		#	print(head(x_lbest))
			...
			#	}
				 ...
				 if(fitness_gbest<(-90) & feat_global_num<=max_num_feats & (global_no_change<=global_max_itr))
				 {
			...
			#d1pct<-100*(d1/length(x_lbest_mean))
				...
				if(k>itr)
				{
						...
						#if(fitness_gbest<(-minfitnessthresh) & (global_no_change>=global_max_itr) & d1pct<=10){
						...
						#}else
						{
							...
							#if(global_no_change<=global_max_itr | len_feat_ind<=minnum)
							{
