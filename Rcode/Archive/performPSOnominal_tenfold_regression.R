...
{	
	...
	eval_fit_kfold<-function(particle, k)
	{
		...
		if(num_feat>1)
		{
			...
			#model<-svm(trainset, trainclass, type="eps", kernel=kname, cross=kcross)

			#model<-svm(trainset, trainclass, type="eps", kernel=kname, degree=tune_degree, gamma=tune_gamma, cost=tune_cost, cross=kcross)
			
			 model<-svm(trainset$trainclass~., data=trainset, type="eps", cross=10) #dim(trainset)[1])
			...
			#fitval<-predict(model, trainset[,-c(1)])
                        ...
                        #folderror<-0.5*(model$tot.MSE+(0.5*(beracc)*100))

			#aic<-2*+dim(ncid)[1]*log(sum((model$fitted-trainclass)^2))
			
			#folderror<-(0.7*model$tot.MSE)+(0.3*mean((model$fitted-trainclass)^2))
			
			#folderror<-(1-ber)*100
			
			folderror<-(cor(trainclass,model$fitted))-model$tot.MSE-(mean((model$fitted-trainclass)^2))
			rm(trainset)
	...
	#model_train_err<-svm(finalset, trainclass,  kernel=kname, type="eps", cross=10)

	#model_train_valid<-svm(finalset, trainclass,   kernel=kname, type="eps")

	model_train_err<-svm(modtrain$Class~., data=modtrain, type="eps", cross=10)

	model_train_valid<-svm(modtrain$Class~., data=modtrain, type="eps")
	...
	print(model_train_err$tot.MSE)
	
	  filestr3<-paste(outloc, "10foldaccuracy.csv", sep="")
        write.table(model_train_err$tot.MSE, file=filestr3, sep=",", row.names=FALSE)



	error<-mean((pred_train-trainclass)^2)  #1-sum(diag(train.table))/(dim(finalset)[1])
	print("Modified train MSE is ")
	print(error)
