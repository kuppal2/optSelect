...
svm_acc[i]<-(beracc*100)
	...
	accuracyweightB=1,max_num_feats=10)
	{
		...
		if(num_feat>1)
		{
			...
			#if(FALSE)
			seed_vec<-c(129532,839147,407700,100,1000,555,9999,123456,83414,1242)
			setseed=NA #seed_vec[1]
                        #model<-svm_cv(v=numfolds,x=trainset[,-1],y=trainset$trainclass,kname=kname,errortype=errortype,setseed=setseed)
                        #folderror_cur_reg<-model$confint[1]
			#folderror<-folderror_cur_reg
			{
			 for(f in 1:10)
                        {
			 #model<-svm_cv(v=numfolds,x=trainset[,-1],y=trainset$trainclass,kname=kname,errortype="AUC")
...
svm_acc[i]<-(beracc*100)
		...
		if(FALSE){
			      folderror_perm<-max(folderror_perm,na.rm=TRUE)+(1.96*(sd(folderror_perm,na.rm=TRUE))) #/sqrt(2)))
                              folderror_test<-folderror 
				if(num_feat>max_num_feats){
                                fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror_test)-(featweight*100*(num_feat/length(particle))) #(featweight*(num_feat))
                                }else{
		...
		#if(FALSE)
		{
				...
				if(t1$p.value<0.05){
					...
					folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(1*(sd(folderror_perm,na.rm=TRUE))) #/sqrt(4))) #max(folderror_perm)
					folderror<-mean(folderror)
					fitfunc<-(accuracyweightA*(folderror-folderror_perm))+accuracyweightB*(folderror)+folderror_test-(featweight*100*(num_feat/length(particle)))
				}else{
					fitfunc<-(-100)
				}	
		}
...
constriction_factor=1,select.global.best=TRUE,numnodes=20,bootstrap.itr=10,evalFunc,boostweight,...)
{
...
boostweight=boostweight,...)
		...
		s1<-summary(summat)
		num_itr_thresh<-round(s1[1]) #ceiling(minselect.pct*globalpso_maxitr)
		...
		testacc_thresh<-summary(testacc,na.rm=TRUE)

		best_fold<-which(testacc>=testacc_thresh[4]) #max(testacc,na.rm=TRUE)[1])
		print(best_fold)
		if(length(best_fold)>1){
		sm1<-scoringmatrix[,best_fold]
			sm2<-apply(sm1,1,mean) #as long as 1 is selected
			bestgenelist<-which(sm2>=minselect.pct)
		}else{
		sm1<-scoringmatrix[,best_fold]
