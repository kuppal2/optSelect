...
outloc<-"/home/stu/kuppal3/Research/SpectF_data/Results/itr1pso100wrand_neigh0.6conf0.1behav0.3_transmatBER_minsel0.5_CV/"
...
svm_acc[i]<-(beracc*100)
	...
	prob_behavior<-c(0.1,0.1,0.8) #rep(c(0.2,0.4,0.4),num_part) #runif(num_part,0,1)
		...
		repeat
		{
			...
			for (i in 1:num_part)
			{
				...
				if(k%%behavior_reset_itr==0){
					...
					prob_actions<-prob_behavior %*% (transition_matrix %^% k)
