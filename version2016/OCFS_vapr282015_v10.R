...
svm_acc[i]<-(beracc*100)
	...
	#function to evaluate k-fold CV
	eval_fit_test_diff_v1<-function(particle, numfolds,trainm,trainclass,testm,testclass,errortype="AUC",kname="radial",featweight=0.05,accuracyweightA=5,
	accuracyweightB=1,max_num_feats=10)
	{
...
svm_acc[i]<-(beracc*100)
		...
		if(FALSE){
				...
				if(t1$p.value<0.05){
					...
					folderror_perm<-mean(folderror_perm,na.rm=TRUE)+(1.96*(sd(folderror_perm,na.rm=TRUE)/sqrt(4))) #max(folderror_perm)
